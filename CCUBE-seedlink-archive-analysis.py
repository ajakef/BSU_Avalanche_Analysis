...
#text = subprocess.check_output(['grep', '172.111.147', 'CC' + CCUBE_ID + '_status.txt'])
...
def handle_data(incomingData):
    ...
    if(t0 > (new_archive_hour + 10)):
        ...
        mseedData.merge()
        if(False):
            filename = 'CCUBE_test/' + dig2(current_archive_hour.year) + \
                '.' + dig2(current_archive_hour.julday) + \
                '.' + dig2(current_archive_hour.hour) + '.' + \
                dig2(current_archive_hour.minute) + '.' + \
                dig2(current_archive_hour.second) + '.' + \
